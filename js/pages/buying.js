					...
					data.forEach(function(item, i, arr) {
						...
						+ '<div class="col-xs-10">'
							+ '<div class="row">'
								...
								+ '<div class="col-xs-5" title="' + chrome.i18n.getMessage("price_difference_buy") + '"><span class="fa fa-sort-amount-asc"></span> <span class="price-range">' + chrome.i18n.getMessage("loading") + '</span></div>'
							+ '</div>'
						+ '</div>'
						+ '<div class="col-xs-1 text-xs-center">'
							+ '<div class="cursor-pointer track-this-item' + (local_storage['buying_track_list'] && local_storage['buying_track_list'][item['id']] ? ' text-info' : '') + '" data-item-id="' + item['id'] + '" data-item-vnum="' + item['item_id'] + '" data-item-count="' + item['quantity'] + '" data-item-price="' + item['price'] + '" title="' + chrome.i18n.getMessage("notify_when_bought") + '">'
	...
	$.ajax({
		...
		success: function(data, textStatus, XMLHttpRequest) {	
			...
			data.forEach(function(item, i, arr) {
				...
				var best_price = item['buys']['unit_price'];
				
				$(".item-block[data-vnum=" + item['id'] + "]").find(".price-range").html(format_coins(best_price - current_price));
			});
		},
		error: function(x, t, m) {
			/* Fix */ if (current_page != "buying") { load_page(current_page, true); return; }
...
$(document).off("click", ".track-this-item").on("click", ".track-this-item", function() {
	...
	chrome.storage.local.get(function(local_storage) {
		if (local_storage['buying_track_list'] && local_storage['buying_track_list'][var_item_id]) {			
			...
			var item_array = local_storage['buying_track_list'] ? local_storage['buying_track_list'] : {};
...
$(document).off("click", "#track_button").on("click", "#track_button", function() {
	if ($(this).data("act") == "track") {
		...
		chrome.storage.local.remove("buying_track_list", function() {
