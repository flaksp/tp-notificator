			...
			$.ajax({
				...
				success: function(data, textStatus, XMLHttpRequest) {
					...
					if (!data || data.length == 0) {
						$("#listing").html('<div style="display: table; width: 100%"><div class="text-xs-center" style="display: table-cell; vertical-align: middle; height: calc(100vh - 2rem)">' + chrome.i18n.getMessage("no_selling_transactions") + '</div></div>');
					...
					data.forEach(function(item, i, arr) {
						...
						+ '<div class="col-xs-10">'
							+ '<div class="row">'
								...
								+ '<div class="col-xs-5" title="' + chrome.i18n.getMessage("price_difference_sell") + '"><span class="fa fa-sort-amount-desc"></span> <span class="js-price-range">' + chrome.i18n.getMessage("loading") + '</span></div>'
							+ '</div>'
						+ '</div>'
						+ '<div class="col-xs-1 text-xs-center">'
							+ '<div class="cursor-pointer js-track-this-item' + (typeof local_storage['selling_track_list'][item['id']] !== "undefined" ? ' text-info' : '') + '" title="' + chrome.i18n.getMessage("notify_when_sold") + '">'
