		...
		success: function(data, textStatus, XMLHttpRequest) {
			$.each(sold_item_ids, function(index, value) {
				...
				chrome.notifications.create("notif_" + Date.now() + 'x' + Math.random(), {
					...
					message: item_name + ' (' + (value['sold_all'] ? chrome.i18n.getMessage("all") : value['count'] + " " + chrome.i18n.getMessage("items")) + '). ' + chrome.i18n.getMessage("profit", [format_coins_clean(Math.ceil(value['price'] / 100 * 85) * value['count'])])
				});
			});
		},
		error: function(x, t, m) {
			...
			$.each(sold_item_ids, function(index, value) {				
				chrome.notifications.create("notif_" + Date.now() + 'x' + Math.random(), {
					...
					title: chrome.i18n.getMessage("item_sold"),
					message: index + ' (' + (value['sold_all'] ? chrome.i18n.getMessage("all") : value['count'] + " " + chrome.i18n.getMessage("items")) + '). ' + chrome.i18n.getMessage("profit", [format_coins_clean(Math.ceil(value['price'] / 100 * 85) * value['count'])])
				});
			});
		}
	});
