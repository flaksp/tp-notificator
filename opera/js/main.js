...
window.onload = function() {
	...
	// Open default page defined via settings
	...
	$(document).on('close.bs.alert', '#vote_reminder_alert', function () {
		chrome.storage.local.set({"vote_remind": true}, function() {
			console.log("Notification hidden forever.");
		});
	});
	
	// Hide headhunt translator notification forever
	$(document).on('close.bs.alert', '#translator_notice_alert', function () {
		chrome.storage.local.set({"translator_notice": true}, function() {
			console.log("Notification hidden forever.");
		});
	});
}
