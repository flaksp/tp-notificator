...
$("body").off("submit", "#add_api_key").on("submit", "#add_api_key", function(event) {
	...
	function get_account_name(api_key, success, error) {}
		...
		success: function(data, textStatus, XMLHttpRequest) {
			if (data['permissions'].indexOf("tradingpost") == -1) {
				...
				$.ajax({
					...
					tryCount: 0,
					retryLimit: 3,
					success: function(data_account, textStatus, XMLHttpRequest) {
						cbf.storage.sync.get(function(sync_storage) {
							var api_key_name = data_account['name'];

							if (data['name'].length > 0) {
								api_key_name += ' (' + data['name'] + ')';
							}

							sync_storage['access_token'][api_key] = api_key_name;
							...
							chrome.storage.local.set({
								...
								cbf.storage.sync.set({
									"access_token": sync_storage['access_token']
								}, function() {
									$(el).find(".js-notifications").html('<p class="text-success">' + chrome.i18n.getMessage("api_key_saved", [api_key_name]) + '</p>');

									if ($("#current_api_key").find("option[value=" + api_key + "]").length > 0) {
										$("#current_api_key").find("option[value=" + api_key + "]").html(api_key_name);
									} else {
										$("#current_api_key select").append('<option value="' + api_key + '">' + api_key_name + '</option>');
									}
...
$("body").off("submit", "#graph_tool").on("submit", "#graph_tool", function(event) {
	...
	chrome.storage.local.set({
		"graph_tool": parseInt(graph_tool)
	}, function() {
...
$("body").off("submit", "#item_language").on("submit", "#item_language", function(event) {
	...
	chrome.storage.local.set({
		"item_localization": item_language
	}, function() {
		$(el).find(".js-notifications").html('<p class="text-success">' + chrome.i18n.getMessage("changes_saved") + '</p>');
	});
});
